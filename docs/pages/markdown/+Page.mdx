import { RepoLink, Link, Warning, Contribution } from '@brillout/docpress'
...
A simple way to define metadata is to define a `metadata.js` file that contains <Link href="#metadata:~:text=global%20or%20local.-,Global%20metadata,-.">global metadata</Link>.
...
You can use <Link href="/meta">`meta`</Link> to create custom settings for defining <Link href="#metadata:~:text=for%20every%20page.-,Local%20metadata,-.">local metadata</Link>.
